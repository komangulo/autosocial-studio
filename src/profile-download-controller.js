...
const { enrichRecentVideos } = require("./video-meta");
    ...
    _runYtDlp(url, minViews, maxVideos, scanOnly) {
        return new Promise((resolve) => {
            ...
            execFile(this.ytDlp, dlArgs, { timeout: 300_000 }, async (err, stdout, stderr) => {
                ...
                if (!scanOnly) {
                    try {
                        const metaSummary = await enrichRecentVideos(
                            this.downloadsDir,
                            startedAtMs,
                            { logger: (message, level = "info") => this.log(message, level) }
                        );
                        if (metaSummary.processed > 0) {
                            this.log(`Wrote metadata sidecar for ${metaSummary.processed} video(s).`);
                        }
                    } catch (metaError) {
                        this.log(`Metadata extraction failed: ${metaError.message}`, "error");
                    }

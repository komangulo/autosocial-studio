...
const { createAutoCloneRouter } = require("./autoclone");
...
function chooseFolder(startPath) {
  return new Promise((resolve) => {
    ...
    const platform = os.platform();
    const candidates = startPath && fsSync.existsSync(startPath) ? [startPath] : [];
    if (platform === "win32") {
      const script = [
        "Add-Type -AssemblyName System.Windows.Forms",
        "$d = New-Object System.Windows.Forms.FolderBrowserDialog",
        "$d.Description = 'Elige la carpeta de videos'",
        ...candidates.map((p) => `$d.SelectedPath = ${JSON.stringify(p)}`),
    ...
    if (platform === "darwin") {
      const script = "POSIX path of (choose folder with prompt \"Elige la carpeta de videos\")";
      const child = spawn("osascript", ["-e", script], { stdio: ["ignore", "pipe", "ignore"] });
      let out = "";
      child.stdout.on("data", (chunk) => { out += chunk.toString(); });
      child.on("error", () => resolve(""));
      child.on("close", () => resolve(out.trim().replace(/\/$/, "")));
      return;
    }
    const tryPicker = (index) => {
      const pickers = [
        ["zenity", ["--file-selection", "--directory", "--title=Elige la carpeta de videos"]],
        ["kdialog", ["--getexistingdirectory", startPath || ".", "Elige la carpeta de videos"]],
      ];
      if (index >= pickers.length) { resolve(""); return; }
      const [bin, args] = pickers[index];
      let settled = false;
      const child = spawn(bin, args, { stdio: ["ignore", "pipe", "ignore"] });
      let out = "";
      child.stdout.on("data", (chunk) => { out += chunk.toString(); });
      child.on("error", () => { if (!settled) { settled = true; tryPicker(index + 1); } });
      child.on("close", (code) => { if (!settled) { settled = true; resolve(code === 0 ? out.trim() : ""); } });
    };
    tryPicker(0);
  });
}
...
function isDirectory(candidate) {
  try { return Boolean(candidate) && require("fs").statSync(candidate).isDirectory(); } catch { return false; }
}
...
async function buildManagedAccountsResponse() {
  ...
  return {
    ...
    onboarding: getAssistedAccountSetupStatus(),
    accounts: await Promise.all(state.accounts.map(async (account) => ({
      ...account,
      credentials: summaryById.get(account.id) || null,
      sessionSaved: await hasSavedPlatformSession("tiktok", account.id),
    }))),
  };
}
...
async function createServer() {
  ...
  // Respect a previous Stop: do not resume a scheduled batch just by opening
  // the app. Explicit actions (Start, "schedule all now") force-start instead.
  ...
  app.use("/api/autoclone", createAutoCloneRouter(express, { getActiveAccount, worker: autonomousWorker }));
  ...
  app.post("/api/start", async (req, res) => {
    autonomousWorker.clearCancellation?.();
    autonomousWorker.start({ force: true });
    const daemons = await getActiveDaemons();
  ...
  app.post("/api/stop", async (req, res) => {
    // Stop the shared TikTok worker too: it may be inside a "schedule all now"
    // drain loop that keeps opening browsers after the daemon timer is cleared.
  ...
  app.post("/api/account-manager/create-assisted", async (req, res) => {
    try {
      const accounts = await getAllAccounts();
      const name = String(req.body?.name || "").trim() || getNextTikTokAccountName(accounts);
      const account = await addAccount(name);
      await getDaemons(account.id);
      const browser = await startAssistedTikTokAccountSetup(account.id);
      res.json({ ok: true, ...browser, manager: await buildManagedAccountsResponse() });
    } catch (error) {
      res.status(400).json({ ok: false, error: error.message });
    }
  });

  app.post("/api/account-manager/open-assisted", async (req, res) => {
    try {
  ...
  app.post("/api/account-manager/confirm-assisted", async (req, res) => {
    try {
      const status = await confirmAssistedTikTokAccountSetup();
      res.json({ ok: true, status, manager: await buildManagedAccountsResponse() });
    } catch (error) {
      res.status(400).json({ ok: false, error: error.message });
    }
  });
  ...
  app.post("/api/uniquifier/start", async (req, res) => {
    try {
      const { inputDir, outputDir, logoImage, intensity, options } = req.body || {};
      const result = await uniquifier.start({ inputDir, outputDir, logoImage, intensity, options });
      res.json(result);
    } catch (error) {
      res.status(400).json({ ok: false, error: error.message });
    }
  ...
  app.post("/api/autoclone/open-folder", async (req, res) => {
    try {
      const controller = require("./autoclone/controller").getAutoCloneController();
      const { jobId } = req.body || {};
      const job = jobId ? await controller.getJob(jobId) : null;
      const folder = job?.folder || controller.lastFolder;
      if (!folder) { res.status(400).json({ ok: false, error: "Todavia no hay ninguna carpeta de descarga." }); return; }
  ...
  app.post("/api/autoclone/choose-folder", async (req, res) => {
    try {
      const { startPath } = req.body || {};
      const folder = await chooseFolder(typeof startPath === "string" ? startPath : "");
      if (!folder) { res.json({ ok: false, cancelled: true }); return; }
      if (!isDirectory(folder)) { res.status(400).json({ ok: false, error: "La ruta elegida no es una carpeta valida." }); return; }
      res.json({ ok: true, folder });
    } catch (error) {
      res.status(400).json({ ok: false, error: error.message });
    }
  ...
  app.post("/api/autoclone/check-folder", async (req, res) => {
    try {
      const { folder } = req.body || {};
      if (!folder || typeof folder !== "string") { res.status(400).json({ ok: false, error: "Escribe una carpeta." }); return; }
      if (!isDirectory(folder)) { res.status(400).json({ ok: false, error: "Esa carpeta no existe en este equipo." }); return; }
      let count = 0;
      for (const entry of await fs.readdir(folder)) {
        if (/\.(mp4|mov|mkv|webm|avi|m4v)$/i.test(entry)) count += 1;
      }
      res.json({ ok: true, folder, videoCount: count });
    } catch (error) {
      res.status(400).json({ ok: false, error: error.message });
    }
  });

...
const { writeMetaFromInfoJson, enrichRecentVideos } = require("./video-meta");
...
class AutoDownloadController {
        ...
        try {
            ...
            if (newFiles.length > 0) {
                ...
                for (const filePath of newFiles) {
                    ...
                    // Turn yt-dlp's rich info.json into our small companion file.
                    await writeMetaFromInfoJson(filePath, { logger: (m, lvl) => this.log(m, lvl) });
    ...
    async _distributeToQueues(videoPath) {
        ...
        // Our companion file has title + description + hashtags already merged.
        const meta = await readVideoMeta(videoPath);
        if (meta?.caption) {
            captionText = meta.caption;
        }
        const metaSourcePath = path.join(parsed.dir, `${parsed.name}.meta.json`);
        ...
        for (const platform of this.platforms) {
            ...
            try {
                ...
                if (meta) {
                    // Copy the sidecar so the publisher can rebuild the caption.
                    await fs.copyFile(metaSourcePath, path.join(queueDir, `${parsed.name}.meta.json`));
                }
                if (captionText) {

...
// MARKER: ACCT-RADAR-UI-v2
// MARKER: ACCT-RADAR-UI-v3
// MARKER: ACCT-RADAR-UI-v4
//
...
(function () {
  ...
  function handle() {
    ...
    // Respaldo: si el campo esta vacio, usa la cuenta abierta/seleccionada en el
    // panel de "Analisis de cuenta" (el boton "Usar esta cuenta" guarda su @).
    if (!v) {
      var useBtn = $("acctUseBtn");
      v = useBtn ? String(useBtn.dataset.handle || useBtn.getAttribute("data-handle") || "").trim() : "";
    }
    v = v.toLowerCase().replace(/[^a-z0-9_.-]/g, "");
    // El radar no necesita una cuenta analizada: publica con la sesion activa de
    // X. Si no hay @ en ningun sitio, usa un perfil unico por defecto.
    return v || "radar";
  }
  ...
  function fill(profile) {
    ...
    if ($("radarLang")) $("radarLang").value = c.language === "es" ? "es" : "en";
  ...
  function save() {
    ...
    var body = {
      ...
      language: $("radarLang") ? $("radarLang").value : "en",
  ...
  function runNow(dry) {
    ...
    return save().then(function () {
      ...
      var scan = r.scan || {};
      var found = Number(scan.found || 0);
      var detail = scan.perKeyword && scan.perKeyword.length
        ? " (" + scan.perKeyword.map(function (k) { return k.keyword + ": " + k.found; }).join(", ") + ")"
        : "";
      if (scan.loginWall) {
        msg("La sesion de X no esta activa en la ventana del radar. Entra en X y pulsa 'Guardar sesion' antes de escanear.", true);
      } else if (dry) {
        msg(pubs.length
          ? ("Encontrado " + pubs.length + " candidato(s) de " + found + " tweets leidos. Ejemplo: " + (pubs[0].text || ""))
          : ("Se leyeron " + found + " tweets" + detail + " pero ninguno paso el filtro (fuera de tema, duplicado o sin link). Revisa tus temas."), found ? false : true);
      } else {
        msg(pubs.length ? ("Publicado(s) " + pubs.length + " post(s) desde el radar.") : ("Se leyeron " + found + " tweets" + detail + ", pero no se publico nada nuevo."), found ? false : true);
      }
      fill(d.profile || {});
      renderHistory(d.profile || {});
  ...
  function stop() {
    ...
    return apiPost("/api/acct-radar/" + encodeURIComponent(h) + "/stop", {})
      .then(function (d) { msg("Radar detenido. No publicara nada mas hoy.", false); fill(d.profile || {}); })
  ...
  function boot() {
    ...
    if ((b = $("radarStop"))) b.addEventListener("click", function () { stop(); });

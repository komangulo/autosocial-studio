...
const { writeMetaFromInfoJson } = require("../src/video-meta");
const { readVideoMeta } = require("../src/queue");
...
async function distributeToQueues(videoPath, accountId) {
    ...
    // Prefer our companion metadata (title + description + hashtags merged).
    ...
    for (const platform of PLATFORMS) {
        ...
        try {
            ...
            if (meta) {
                await fsp.copyFile(metaSourcePath, path.join(queueDir, `${parsed.name}.meta.json`));
            }
            if (captionText) {
...
async function poll(accountId) {
    ...
    if (newFiles.length > 0) {
        ...
        for (const filePath of newFiles) {
            ...
            await writeMetaFromInfoJson(filePath, { logger: (m) => log(m) });
